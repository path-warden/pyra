...
const gitLogFormat = "--format=" + recSep + "%H" + fieldSep + "%an" + fieldSep + "%ct" + fieldSep + "%s"
...
func walk(root string, window int) (recs []commitRec, asOf int64, capped bool, ok bool) {
	out, err := exec.Command("git", "-C", root,
		"log", "-n"+itoa(window), "--no-merges", "--no-renames", "--date-order", gitLogFormat, "--numstat").Output()
	if err != nil {
...
func parseLog(raw string) []commitRec {
	...
	for _, block := range strings.Split(raw, recSep) {
		...
		header := strings.SplitN(lines[0], fieldSep, 4)
		if len(header) < 3 {
			continue
		}
		rec := commitRec{
