...
capabilities that are a genuine fit for Pyra's model — **deterministic,
...
Pyra is an **authority layer**: Canon is enforced truth, the gate blocks
...
We deliberately **exclude** repowise's LLM wiki/RAG (Pyra's Reference tier +
...
### Non-negotiable constraints (inherited from Pyra's invariants)
...
2. **Implementation & licensing.** Repowise is **AGPL-3.0**. Default posture is an
   ...
   **Exception (change-risk, by explicit decision):** to be parity-testable against
   ...
   AGPL terms — relevant if Pyra is distributed under other terms — and are meant
   to be replaced by Pyra-owned constants later. Pyra currently ships **no
...
3. **Honesty over headline.** Where repowise ships a defect-calibrated absolute
   score, Pyra leads with **repo-relative ranking** (percentile / tercile
...
module rollups — surfaced via `pyra hotspots` / `pyra ownership` and the
...
- **PR directives** (advisory findings, escalatable by policy):
  ...
  Accepted Canon.
- **Integration**: a `--risk` flag on `pyra gate` (and a standalone
  `pyra risk <range>`), emitting one risk summary + directive findings that
...
unreachable set #5 consumes). Surfaced via `pyra graph`
...
(pyra-unique): `ungoverned_hotspot`, `stale_governance`, `contradictory_decision`.
Surfaced via `pyra health` (`--file`/`--coverage`/`--json`) and `get_health`.
...
graph-aware refactoring *code generation* (pyra names the fix, no codegen).
...
`Test*`/`main`. Surfaced via `pyra dead-code` (`--tier`/`--json`) and
