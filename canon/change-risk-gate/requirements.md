...
Pyra's change-aware gate answers *"which Accepted Canon governs this diff?"*
...
This feature adds a **change-risk** signal to Pyra. Given a change (the staged
...
`--risk` flag on `pyra gate` (findings merged into the one gate result and exit
code) and as a standalone `pyra risk <range>` command.
...
on `pyra gate` that merges into the existing `Result` / exit code / JSON /
SARIF; a standalone `pyra risk [range]` command.
...
to offline calibration, not the runtime path); training Pyra's *own* calibration
...
- [REQ-101] WHEN a change is scored THEN Pyra SHALL compute the Kamei just-in-time metrics of the diff: lines added, lines deleted, files touched, distinct directories touched, distinct top-level subsystems touched, and the Shannon entropy of the per-file churn distribution.
...
- [REQ-201] WHEN a change is scored THEN Pyra SHALL report a repo-relative review priority of `Below typical`, `Typical`, or `Elevated`, derived from terciles of the repository's own recent-commit risk distribution.
- [REQ-202] WHEN a change is scored THEN Pyra SHALL report the percentile of the change within the repository's own recent-commit risk distribution.
- [REQ-203] WHEN the raw 0–10 score is displayed THEN Pyra SHALL present it as a secondary, clearly-labeled number and SHALL NOT present it as a portable or defect-calibrated verdict.
- [REQ-204] WHEN the recent-commit distribution is sampled THEN Pyra SHALL sample a bounded number of recent commits, and the sample size SHALL be configurable with a documented default.
- [REQ-205] IF too few commits exist to form a stable distribution THEN Pyra SHALL report the ranking as unavailable and still report the raw score, rather than emitting a misleading percentile.

### Requirement 3 — Actionable PR directives

- [REQ-301] WHEN a changed source file has no corresponding test file present in the same change THEN Pyra SHALL emit a `missing_tests` directive naming that file.
- [REQ-302] WHEN a changed file has recent co-change partners that are absent from the change THEN Pyra SHALL emit a `missing_cochanges` directive naming those partners.
- [REQ-303] WHEN a changed symbol has structural dependents (callers/importers) resolvable via code intelligence THEN Pyra SHALL emit a `will_break` directive naming the dependents that may be affected.
- [REQ-304] WHEN a change touches code governed by Accepted Canon THEN Pyra SHALL emit a `governance_risk` directive citing the governing artifact, reusing the change-aware gate's governance resolution rather than recomputing it.
- [REQ-305] WHEN a directive is emitted THEN Pyra SHALL classify it as advisory by default and SHALL allow the enforcement policy to reclassify it as blocking or disabled by its stable rule code.

### Requirement 4 — Minimal git-intelligence substrate

- [REQ-401] WHEN change-risk needs history THEN Pyra SHALL derive per-file churn, co-change pairs, and author commit counts from `git log` over a bounded, configurable history window, in a package outside `internal/canon/...`.
- [REQ-402] WHEN co-change pairs are computed THEN Pyra SHALL count files that changed together in the same commit, and SHALL exclude pairs that are already linked by a code-intelligence import edge so that only hidden coupling is reported.
...
- [REQ-501] WHEN `pyra gate` is run with the risk flag THEN Pyra SHALL compute change-risk over the same change set the change-aware gate uses and SHALL merge the risk directives into the single gate `Result`, exit code, JSON, and SARIF outputs.
- [REQ-502] WHEN `pyra risk [range]` is run THEN Pyra SHALL score the given change (defaulting to the staged diff) and render the ranking, raw score, drivers, and directives, with a machine-readable output option.
...
- [REQ-504] WHERE the risk flag is not passed to `pyra gate` THEN the gate SHALL produce output byte-identical to today for identical repository state.
...
- [REQ-602] WHEN change-risk runs THEN Pyra SHALL perform no network access and SHALL invoke no LLM.
- [REQ-603] WHEN change-risk runs on identical repository state and identical change input THEN Pyra SHALL produce identical output, independent of map iteration order, time, or randomness.
- [REQ-604] IF code intelligence is unavailable for a changed file (unsupported language, missing file) THEN Pyra SHALL skip the `will_break` computation for that file and continue, rather than failing the run.
